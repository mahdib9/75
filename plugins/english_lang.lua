...
local function run(msg, matches)
	if permissions(msg.from.id, msg.to.id, "lang_install") then
		...
		set_text(LANG, 'commands:1', '!help: Show all commands for every plugin.')
		set_text(LANG, 'commands:2', '!Help [plugin]: Commands for that plugin.')
		...
		set_text(LANG, 'giverank:4', '1rank mod <user_id>/<user_name>: add mod by user ID/Username.')
		...
		set_text(LANG, 'moderation:17', '1unmute <id>/<username>: the user will be unsilenced by its ID/Username in the current supergroup.')
		...
		set_text(LANG, 'plugins:2', '1plugins <enable>/<disable> [plugin]: enable/disable the specified plugin.')
